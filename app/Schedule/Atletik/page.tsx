...
const SchedulePage: React.FC = () => {
    ...
    const selectedSchedule = schedule.find((s): s is Schedule => s.round === selectedRound && s.match !== undefined);
      ...
      <section>
        <div className='w-full min-h-screen h-full bg-[#F9F3BA]'>
          <div className='p-4 md:p-10'>
            <Link href="/Schedule">
                <h3 className='bonjour-font text-[#102F8E] text-xl'> {'<'} Kembali </h3>
            </Link>
                ...
                {[1, 2, 3, 4, 5, 6, 7].map((round) => (
                    <button
                        ...
                        className={`px-4 py-1 text-xs sm:text-base md:text-lg rounded-xl bonjour-font ${
                            selectedRound === round ? 'bg-[#1D48B9] text-white' : 'bg-white text-[#1D48B9] border-[3px] border-[#1D48B9]'
                        }`}
                    >
                        {round == 1 && "Round 1"}
                        {round == 2 && "Round 2"}
